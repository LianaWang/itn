...
from utils.overlp import coordinates_transfer
import math
...
def _load_text_annotation(index):
    ...
    filename = os.path.join('/mnt/disk/wangfangfang/synthetic/annotations/', str(index) + '.txt')
...
def vis_results(imname, gt_boxes):
    filename = os.path.join('/mnt/disk/wangfangfang/synthetic/SynthText/', imname)
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig('./tools/result/' + imname[2:])
