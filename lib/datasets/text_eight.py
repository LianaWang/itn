...
class text_eight(imdb):
    def __init__(self, image_set, devkit_path=None):
        rname=cfg.DATA_DIR[:-1]
        imdb.__init__(self, rname[rname.rfind('/')+1:])
    ...
    def image_path_from_index(self, index):
        ...
        """
        image_path = os.path.join(self._data_path, 'JPGImages',
    ...
    def _load_text_annotation(self, index):
        ...
        format.
        """
        filename = os.path.join(self._data_path, 'Annotations', index + '.gt')
        ...
        for ix in range(num_objs):
            ...
            gt_classes[ix] = cls
            hard_flg[ix] = int(objs[0])
            overlaps[ix, cls] = 1.0

        idx_easy = np.where(hard_flg[:] == 0)[0]
        boxes = boxes[idx_easy, :]
        gt_classes = gt_classes[idx_easy]
        hard_flg = hard_flg[idx_easy]
        overlaps = overlaps[idx_easy, :]

        import utils.bbox_overlap as bbox_olp
        seg_areas = bbox_olp.gpu_areas(boxes)
